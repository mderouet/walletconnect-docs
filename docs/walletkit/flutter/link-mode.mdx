...
**WalletKit Link Mode** is a low latency mechanism for transporting One-Click Auth requests over Universal Links, eliminating the need for a WebSocket connection with the Relay. This significantly enhances the user experience when connecting native dApps to native wallets by reducing the latency associated with networking connections, especially when the user has an unstable internet connection.

By enabling it, wallet and dapp will communicate through declared Universal Links on iOS and/or App Links on Android **even without internet connection.**
...
1. Add an Universal Link for your wallet in the **Explorer** tab of your [**Cloud project configuration**](https://cloud.walletconnect.com/sign-in), under **Mobile Linking** section
...
On Flutter there are different plugins that would help you integrate Universal/App Links but, no matter which one you choose, it is important that, when capturing an incoming link, you pass it to WalletKit so it can process it:
...
2. Ensure that you enabled the Associated Domains Capability in your XCode project and that your Universal Link is properly configured. _(Depending on the preivous states of your Provisioning Profiles it may be required to update/create new ones)_
```xml
...
1. Ensure to handle incoming App Links in your Activity's `onCreate` method and in `onNewIntent` callback.
